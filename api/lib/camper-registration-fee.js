 ...
 * campers.registration_fee_paid, campers.registration_fee_waived_ima_member (IMA member waive),
 * or any confirmed / pending_step_up row with registration_fee_paid
...
async function isCampRegistrationFeePaid(sb, camperId) {
  ...
  const { data: camp, error: ce } = await sb
    .from('campers')
    .select('registration_fee_paid, registration_fee_waived_ima_member')
    .eq('id', id)
    .maybeSingle();
  if (ce) throw ce;
  if (camp && camp.registration_fee_paid === true) return true;
  if (camp && camp.registration_fee_waived_ima_member === true) return true;
