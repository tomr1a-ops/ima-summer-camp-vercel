-- =============================================================================
-- IMA Summer Camp — COMPLETE schema (single run in Supabase SQL Editor)
-- Use on a NEW project, or drop existing public tables first if re-installing.
-- =============================================================================
...
-- parent_id nullable for guest checkout (server creates camper before parent account)
CREATE TABLE public.campers (
  ...
  extra_shirt_addon_paid BOOLEAN NOT NULL DEFAULT false,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
-- parent_id nullable for guest checkout; guest_email set after Stripe payment if no parent
...
-- =============================================================================
-- After run: add SUPABASE_SERVICE_KEY to Vercel (Settings → API → service_role)
-- =============================================================================
