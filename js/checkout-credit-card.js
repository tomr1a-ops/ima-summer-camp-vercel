/**
 * Standalone credit-card camp checkout (no Step Up).
 * Restores draft from same localStorage key as index.html; validates parent/weeks/children query params.
 */
(function () {
  ...
  var lastSummaryStripeSessionDollars = 0;
  ...
  async function renderOrder() {
    var elErr = document.getElementById('cc-error');
    ...
    if (!session) {
      if (elEmpty) {
        ...
        elEmpty.innerHTML =
          ...
          encodeURIComponent('/checkout-credit-card.html' + location.search) +
    ...
    lastSummaryStripeSessionDollars = grand;
  ...
  async function startPay() {
    var btn = document.getElementById('pay-btn');
    var msg = document.getElementById('cc-error');
    function err(t) {
      if (msg) {
    ...
    try {
      ...
      if (r.skip) {
        err('Nothing to pay. Return to camp registration and select weeks or an add-on.');
        return;
      }
      if (r.error) {
        err(r.error);
      ...
      var stripePk = '';
      try {
        stripePk = await IMA.getStripePublishableKey();
      } catch (eCfg) {
        err('Could not load site configuration.');
        return;
      }
      if (!stripePk) {
        err('Payment configuration missing. Contact IMA.');
        return;
      }
      if (typeof window.Stripe !== 'function') {
        err('Stripe could not load. Disable blockers and refresh.');
        return;
      }
      var stripe = window.Stripe(stripePk);
      var data = r.data;
      try {
        if (data && data.batchId) sessionStorage.setItem('ima_stripe_pending_batch', String(data.batchId));
      } catch (eSt) {}
      if (!data.sessionId || typeof data.sessionId !== 'string') {
        err('Checkout did not return a Stripe session. Try again or contact IMA.');
        return;
      }
      if (data.totals != null && data.totals.total != null) {
        var serverT = Number(data.totals.total);
        var clientT = Number(lastSummaryStripeSessionDollars);
        if (Number.isFinite(serverT) && Number.isFinite(clientT) && Math.abs(serverT - clientT) > 0.02) {
          await renderOrder();
          err(
            'Your order total was updated to $' +
              serverT.toFixed(2) +
      ...
      var hosted = data.checkoutUrl && /^https:\/\//i.test(String(data.checkoutUrl));
      if (hosted) {
        window.location.assign(String(data.checkoutUrl));
        return;
      }
      var out = await stripe.redirectToCheckout({ sessionId: data.sessionId });
      if (out.error) err(out.error.message);
    } catch (e) {
      err(
        e && e.name === 'AbortError'
          ? 'Request timed out.'
  ...
  function start() {
    ...
    if (pay) pay.addEventListener('click', function () { void startPay(); });
    void boot();
  }
